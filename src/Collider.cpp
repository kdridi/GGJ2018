...
bool Collider::test(const sf::Vector2f &v)
{
  ...
  if (ColliderMap::current->isCollide({pos.x + v.x,
	  pos.y + v.y}) == false &&
    ColliderMap::current->isCollide({pos.x + s.x + v.x,
	  pos.y + v.y}) == false &&
    ColliderMap::current->isCollide({pos.x + v.x,
	  pos.y + s.y + v.y}) == false &&
    ColliderMap::current->isCollide({pos.x + s.x + v.x,
	  pos.y + s.y + v.y}) == false)
    {
      if (ColliderMap::current->isCollide({pos.x + v.x + s.x/2,
	      pos.y + v.y}) == false &&
	ColliderMap::current->isCollide({pos.x + s.x + v.x,
	      pos.y + v.y + s.y/2}) == false &&
	ColliderMap::current->isCollide({pos.x + v.x + s.x/2,
	      pos.y + s.y + v.y}) == false &&
	ColliderMap::current->isCollide({pos.x + v.x,
	      pos.y + s.y/2 + v.y}) == false)
	return (false);
    }
  return (true);
}
